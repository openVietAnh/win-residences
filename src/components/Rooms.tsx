...
const oneBedroomImages = import.meta.glob(
  "/src/assets/1bedroom/**/*.{jpg,JPG,jpeg,JPEG,png,PNG}",
  { eager: true, as: "url" }
) as Record<string, string>;

const twoBedroomImages = import.meta.glob(
  "/src/assets/2 bedrooms/**/*.{jpg,JPG,jpeg,JPEG,png,PNG}",
  { eager: true, as: "url" }
) as Record<string, string>;

const commonAreaImages = import.meta.glob(
  "/src/assets/Common areas/**/*.{jpg,JPG,jpeg,JPEG,png,PNG}",
  { eager: true, as: "url" }
) as Record<string, string>;
...
const Rooms = () => {
  ...
  const oneBedroomGroups = useMemo(
    () => groupImagesByFolder(oneBedroomImages),
    []
  );
  const twoBedroomGroups = useMemo(
    () => groupImagesByFolder(twoBedroomImages),
    []
  );
  const commonAreaGroups = useMemo(
    () => groupImagesByFolder(commonAreaImages),
    []
  );
    ...
    () => [
      {
        ...
        subSections: Object.entries(oneBedroomGroups).map(
          ([title, images]) => ({
            title,
            images,
          })
      ...
      },
      {
        ...
        subSections: Object.entries(twoBedroomGroups).map(
          ([title, images]) => ({
            title,
            images,
          })
      ...
      },
      {
        ...
        subSections: Object.entries(commonAreaGroups).map(
          ([title, images]) => ({
            title,
            images,
          })
